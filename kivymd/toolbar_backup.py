...
<Toolbar>:
    ...
    middle_content:
        ...
        MDLabel:
            ...
            text: root.title if not root.logo else ''
            shorten: True
            shorten_from: 'right'
    BoxLayout:
        id: right_actions
...
class middle_content(BoxLayout):
    logo_offset_x = NumericProperty('20dp')
    logo_offset_y = NumericProperty('3dp')
    logo_x = NumericProperty(100)
    logo_y = NumericProperty(100)
    ...
    def _update_size(self, *args):
        self.logo_size = (self.texture.size[0] * (self.size[1] - (self.height - dp(48)))) / self.texture.size[1], \
                         self.size[1] - (self.height - dp(48))
        self.logo_pos = (self.pos[0] + dp(0), self.pos[1])
